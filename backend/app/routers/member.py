        ...
        "members": [
            {
                ...
                "person_name": mem.person.full_name if mem.person else None,
                "role": mem.role,
...
async def add_ministry_member(
    ...
    """Leader adds a member to their ministry."""
    ...
    # Add user_role for login access
    existing_role = (
        db.query(UserRole)
        .filter(
            UserRole.user_email == body.email,
    ...
    return {
        "id": membership.id,
        "person_id": person.id,
        "person_name": person.full_name if hasattr(person, "full_name") else body.name,
        "role": membership.role,
        "joined_date": membership.joined_date.isoformat(),
        "message": f"{body.email} added to {ministry.name}",
    }
