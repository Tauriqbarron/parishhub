<script lang="ts">
	...
	let settings: RosterTemplateSettings = $state({
		keep_assignee: false,
		auto_open_hours: 168,
	...
	async function handleDuplicate() {
		if (!template) return;
		try {
			await rosterApi.duplicateTemplate(template.id);
			goto('/rosters/templates');
		} catch {}
	}
</script>
...
{:else}
	<div class="page">
		<button class="back-btn" onclick={() => goto('/rosters/templates')}
			><ArrowLeft class="icon-sm" /> Back to Templates</button
		>
		<div class="header-row">
			<h1>Edit: {template?.name}</h1>
			<button class="btn-secondary" onclick={handleDuplicate}
				><Copy class="icon-sm" /> Save as New</button
			>
		</div>
		...
		<section>
			...
			<label class="toggle-label">
				<input
					type="checkbox"
					checked={is_active}
					onchange={(e) => (is_active = e.currentTarget.checked)}
				/>
				Active
			</label>
		</section>
		...
		<section>
			...
			{#each slots as slot}
				<div class="slot-row">
					...
					<select
						value={slot.role_id}
						onchange={(e) => updateSlot(slot.id, 'role_id', Number(e.currentTarget.value))}
					>
					...
					<input
						type="text"
						value={slot.label}
						oninput={(e) => updateSlot(slot.id, 'label', e.currentTarget.value)}
						placeholder="Label"
					/>
					<input
						type="number"
						value={slot.min_persons}
						oninput={(e) => updateSlot(slot.id, 'min_persons', Number(e.currentTarget.value))}
						min={1}
						style="width:4rem"
					/>
					<input
						type="number"
						value={slot.max_persons}
						oninput={(e) => updateSlot(slot.id, 'max_persons', Number(e.currentTarget.value))}
						min={1}
						style="width:4rem"
					/>
					<button class="icon-btn danger" onclick={() => removeSlot(slot.id)}
		...
		<section>
			...
			<label class="toggle-label">
				<input
					type="checkbox"
					checked={settings.keep_assignee}
					onchange={(e) => (settings = { ...settings, keep_assignee: e.currentTarget.checked })}
				/>
			...
			<label class="toggle-label">
				<input
					type="checkbox"
					checked={settings.allow_self_assign}
					onchange={(e) => (settings = { ...settings, allow_self_assign: e.currentTarget.checked })}
				/>
			...
			<label>
				...
				<select
					...
					onchange={(e) =>
						(settings = { ...settings, auto_open_hours: Number(e.currentTarget.value) * 24 })}
				>
				...
				<span class="helper"
					>Roster automatically opens for members this far ahead of the event</span
				>
			</label>
		</section>
